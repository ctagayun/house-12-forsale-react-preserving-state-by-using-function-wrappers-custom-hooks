...
Concepts to Remember:
 ...
 useEffect hook - whenever we use something that's outside the realm of React
  ...
  2. What is a React component?
     It's a function which receives an input (e.g. props) and returns 
  the displayed HTML elements (view). Under the hood, the function 
  only needs to use the rendering mechanism of the render() method 
  ...
  3. Arrow Function
    ...
    JavaScript ES6 arrow function   (NO RETURN - Implicit return)
  ...
  ===================================================
  Current Task: Function Wrappers and Callback Hooks

    https://app.pluralsight.com/ilx/video-courses/9a3771fa-626e-4708-8634-c49cc8616922/2e8c1f61-a9fa-48be-9e6c-4636fc0e17e
    - create away to persist state when the user navigates to
      HouseDetail component in which case the state in the HouseList
      and HouseRow components are unmounted and lost.

    - Create  setSelectedHouseWrapper (A)
  ===============================================
Previous Task:
...
const App = () => {
 ...
 STEP 4:The second state transition we want to handle using using 
  ...
  const searchedHouses = houses.filter((house) =>
   ...
   //(A) - create a wrapper function that take in "searchedHouses"
   //object as Props and pass this to HouseList (B)
   //like this: <HouseList list={setSelectedHouseWrapper} (B)

   //The advantage of using wrapper is that the App.jsx component
   //is still in full control of its state. Because the 
   //setSelectedHouse updater function remains encapsulated
   //in the component
   const setSelectedHouseWrapper = (searchedHouses) => {
      //do checks on searchedHouses to make sure  
      //that it has the properties we are expecting;
      const mySearchedHouses = JSON.stringify(searchedHouses);
      console.log("My SearchedHouses = " + mySearchedHouses );
      setSelectedHouse(searchedHouses) //pass the "searchedHouses" 
                              //object to setSelectedHouse state 
                              //updater function.
      }
  return (
    <>
     ...
     <Search 
      ...
      {selectedHouse ? (
          ...
          <HouseList list={setSelectedHouseWrapper} //(B)
                                           //replace "searchHouses"
                                           //with setSelectedHouseWrapper
